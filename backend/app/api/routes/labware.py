...
# ============================================================================
# LEGACY LABWARE (Old model - kept for backwards compatibility)
# ============================================================================
...
class LabwareCreate(BaseModel):
    ...
    wells: Optional[int] = Field(None, ge=1, le=384)
...
async def list_labware(
    ...
    """Get all labware items with optional filtering"""
    try:
        query = select(Labware)

        if type:
            query = query.where(Labware.type == type)
        if is_active is not None:
            query = query.where(Labware.is_active == is_active)

        query = query.order_by(Labware.name)

        result = await db.execute(query)
        labware_list = result.scalars().all()

        return ApiResponse.success(
            data=[LabwareResponse.model_validate(lw.to_dict()).model_dump() for lw in labware_list],
            message="Labware retrieved successfully"
        )
    except Exception as e:
        return ApiResponse.error(message=f"Failed to get labware: {str(e)}", status_code=500)


@router.post("/seed")
async def seed_labware(db: DbSession = Depends(get_db)):
    """Seed the database with default labware items"""
    # Check if labware already exists
    result = await db.execute(select(Labware))
    existing = result.scalars().all()

    if existing:
        return ApiResponse.success(
            data=[LabwareResponse.model_validate(lw.to_dict()).model_dump() for lw in existing],
            message="Labware already seeded"
        )
    ...
    default_labware = [
        ...
        },
        {
            ...
            "type": "tip_rack",
            "subtype": "50uL",
            "description": "Low-volume 96-tip rack (50µL)",
            "wells": 96,
            "volume": 50,
            "height": None,
        ...
        },
        {
            ...
            "type": "reservoir",
            "subtype": "15mL",
            "description": "15mL trough reservoir",
            "wells": 1,
            "volume": 15000,
            "height": 43,
        ...
        },
        {
            ...
            "type": "reservoir",
            "subtype": "50mL",
            "description": "50mL trough reservoir",
            "wells": 1,
            "volume": 50000,
            "height": 58,
        ...
        {
            "name": "1.5mL Tube Rack",
            "type": "tube",
            "subtype": "1.5mL",
            "description": "Microcentrifuge tube rack (1.5mL)",
            "wells": 24,
            "volume": 1500,
            "height": 45,
        ...
        },
        {
            ...
            "type": "plate",
            "subtype": "deep_96",
            "description": "Deep well 96-well plate (2mL)",
            "wells": 96,
            "volume": 2000,
            "height": 45,
    ...
    created_labware = []
    for lw_data in default_labware:
        new_labware = Labware(**lw_data)
        db.add(new_labware)
        created_labware.append(new_labware)

    await db.commit()

    for lw in created_labware:
        await db.refresh(lw)

    return ApiResponse.success(
        data=[LabwareResponse.model_validate(lw.to_dict()).model_dump() for lw in created_labware],
        message="Labware seeded successfully",
        status_code=201
    )


@router.get("/{labware_id}")
async def get_labware(labware_id: int, db: DbSession = Depends(get_db)):
    """Get a specific labware item by ID"""
    try:
        result = await db.execute(select(Labware).where(Labware.id == labware_id))
        labware = result.scalar_one_or_none()

        if not labware:
            return ApiResponse.not_found(message="Labware not found", resource_type="labware")

        return ApiResponse.success(
            data=LabwareResponse.model_validate(labware.to_dict()).model_dump(),
            message="Labware retrieved successfully"
        )
    except Exception as e:
        return ApiResponse.error(message=f"Failed to get labware: {str(e)}", status_code=500)
...
async def create_labware(labware: LabwareCreate, db: DbSession = Depends(get_db)):
    """Create a new labware item"""
    try:
        new_labware = Labware(**labware.model_dump())
        ...
        return ApiResponse.success(
            data=LabwareResponse.model_validate(new_labware.to_dict()).model_dump(),
            message="Labware created successfully",
            status_code=201
        )
    except Exception as e:
        await db.rollback()
        return ApiResponse.error(message=f"Failed to create labware: {str(e)}", status_code=500)


@router.put("/{labware_id}")
async def update_labware(
    labware_id: int,
    labware: LabwareUpdate,
    db: DbSession = Depends(get_db)
):
    """Update an existing labware item"""
    try:
        result = await db.execute(select(Labware).where(Labware.id == labware_id))
        existing_labware = result.scalar_one_or_none()

        if not existing_labware:
            return ApiResponse.not_found(message="Labware not found", resource_type="labware")

        update_data = labware.model_dump(exclude_unset=True)
        for field, value in update_data.items():
            setattr(existing_labware, field, value)

        existing_labware.updated_at = datetime.utcnow()

        await db.commit()
        await db.refresh(existing_labware)

        return ApiResponse.success(
            data=LabwareResponse.model_validate(existing_labware.to_dict()).model_dump(),
            message="Labware updated successfully"
        )
    except Exception as e:
        await db.rollback()
        return ApiResponse.error(message=f"Failed to update labware: {str(e)}", status_code=500)


@router.delete("/{labware_id}")
async def delete_labware(labware_id: int, db: DbSession = Depends(get_db)):
    """Delete a labware item"""
    try:
        result = await db.execute(select(Labware).where(Labware.id == labware_id))
        existing_labware = result.scalar_one_or_none()
        ...
        await db.execute(delete(Labware).where(Labware.id == labware_id))
        await db.commit()

        return ApiResponse.success(data={"deleted": True}, message="Labware deleted successfully")
    except Exception as e:
        await db.rollback()
        return ApiResponse.error(message=f"Failed to delete labware: {str(e)}", status_code=500)
